...
public class PlayerController implements Initializable {
    ...
    private void handleReady() {
        ...
        lengthText.setText(df.format(duration.toMillis())); //format to only show one decimal place
    ...
    @FXML
    private void handleChangeSlide(MouseEvent me) {
        timeSlider.valueProperty().addListener((Observable ov) -> {
            if (timeSlider.isValueChanging()) {
                if (mediaPlayer != null){
                    ...
                    currentText.setText(df.format(timeSlider.getValue())); //formatted to only show one decimal
                    
                    //Wasnt able to get the media player to play from current spot 
                    //tried setting the start play value to the new time but it still always started playing from
                    //the beginning
                }
